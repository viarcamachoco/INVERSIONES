...
export async function fetchNewsData(query: NewsQueryParams): Promise<NewsDataServiceResult> {
  const cleanQuery: NewsQueryParams = {
    ...
    limit: clampLimit(query.limit),
    includeFallback: query.includeFallback ?? process.env.NEWS_DEMO_FALLBACK === "true"
  };
  ...
  if (articles.length === 0 && !cleanQuery.includeFallback) {
    diagnostics.push({
      ...
      status: "SKIPPED",
      items: 0,
      message: "Demo fallback disabled; returning neutral no-data result instead of synthetic headlines.",
      elapsedMs: 0
    });
  }
