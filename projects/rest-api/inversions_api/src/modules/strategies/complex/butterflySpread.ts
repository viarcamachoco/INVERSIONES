// FIC: Butterfly Spread API route - uses real Alpaca data via shared handler.
// No default portfolio, no hardcoded premiums.
//
// FIC: Ruta API de Butterfly Spread - usa datos reales de Alpaca via handler compartido.
...
export const butterflySpreadRouter = Router();

butterflySpreadRouter.post("/butterfly-spread", authContextMiddleware, async (req, res) => {
  try {
    ...
    const result = await buildStrategyFromChain("butterfly_spread", body);
