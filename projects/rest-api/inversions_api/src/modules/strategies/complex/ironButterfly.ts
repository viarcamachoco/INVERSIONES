// FIC: Iron Butterfly API route - uses real Alpaca data via shared handler.
// No default portfolio, no hardcoded premiums.
//
// FIC: Ruta API de Iron Butterfly - usa datos reales de Alpaca via handler compartido.
...
export const ironButterflyRouter = Router();

ironButterflyRouter.post("/iron-butterfly", authContextMiddleware, async (req, res) => {
  try {
    ...
    const result = await buildStrategyFromChain("iron_butterfly", body);
