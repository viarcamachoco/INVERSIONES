// FIC: Iron Condor API route - uses real Alpaca data via shared handler.
// No default portfolio, no hardcoded premiums.
//
// FIC: Ruta API de Iron Condor - usa datos reales de Alpaca via handler compartido.
...
export const ironCondorRouter = Router();

ironCondorRouter.post("/iron-condor", authContextMiddleware, async (req, res) => {
  try {
    ...
    const result = await buildStrategyFromChain("iron_condor", body);
