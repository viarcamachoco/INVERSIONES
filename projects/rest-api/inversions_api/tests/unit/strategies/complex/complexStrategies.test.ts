...
describe("Complex Options Strategy Engines", () => {
  ...
  describe("Iron Condor", () => {
    ...
    it("net credit = ", () => { const p = e.calculateProfile(buildICConfig()); expect(p.credito_neto).toBe(200); expect(p.tipo_neto).toBe("credito"); });
    it("max gain = ", () => { expect(e.calculateProfile(buildICConfig()).ganancia_maxima).toBe(200); });
    it("max loss = ", () => { expect(e.calculateProfile(buildICConfig()).perdida_maxima).toBe(300); });
  ...
  describe("Butterfly Spread", () => {
    ...
    it("net debit = ", () => { const p = e.calculateProfile(buildBSCfg()); expect(p.credito_neto).toBe(-160); expect(p.tipo_neto).toBe("debito"); });
    it("max loss = ", () => { expect(e.calculateProfile(buildBSCfg()).perdida_maxima).toBe(160); });
